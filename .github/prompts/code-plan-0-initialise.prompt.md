...
# code-plan-0-constitution (alias: phase-0-constitution)
...
You are updating the project constitution at `$$MEMORY-DIR$$/constitution.md` **and** keeping the canonical doctrine files in sync:
...
1) Resolve repository paths
   - Set constants:
     CONST = `$$MEMORY-DIR$$/constitution.md`
     ...
     TMPL  = `$$TEMPLATES-DIR$$/`  # Optional helper content if present
...
3) Gather project doctrine inputs
   ...
   - If test approach is not specified by the user or unknown, ask the user to confirm test approach. eg. TDD (Test first) or traditional (Test last)
...
4) Draft **/$$MEMORY-DIR$$/constitution.md**
   - Use the `$$TEMPLATES-DIR$$/constitution-template.md` as a as a template to replace all place holders and store the actual file to be used as `$$MEMORY-DIR$$/constitution.md`
...
7) Propagate doctrine into helpers (if any)
   - For each file under `$$TEMPLATES-DIR$$` or `$$AGENT-DIR$$/commands/` or `$$AGENT-DIR$$/memory` that references the constitution or rules, ensure links remain correct and language stays stack-neutral.
...
- `$$AGENT-DIR$$/memory/constitution.md` is fully populated, versioned, and includes a Sync Impact Report.
...
- Constitution: `$$MEMORY-DIR$$/constitution.md`
- Rules: `docs/rules-idioms-architecture/rules.md`
- Idioms: `docs/rules-idioms-architecture/idioms.md`
...
Next step (when happy): Run **/code-plan-1-create-specification** to capture the feature specification.
